...
def serialize_post_optimized(post):
    return {
        ...
        'published_at': post.published_at,
        'slug': post.slug,
        'tags': [serialize_tag_optimized(tag) for tag in post.tags.popular().all()],
        'first_tag_title': post.tags.all()[0].title,
    }
...
def index(request):
    ...
    fresh_posts = posts.annotate(comments_count=Count('comment')).order_by('published_at')
...
def tag_filter(request, tag_title):
    ...
    related_posts = tag.posts.annotate(comments_count=Count('comment'))[:20]
